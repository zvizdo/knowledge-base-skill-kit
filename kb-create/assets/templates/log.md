# LOG — {{KB_NAME}}

> Reverse-chronological record of all operations. Newest entries first. Entries older than 7 days are compacted into weekly summaries during maintenance.

## [{{CREATED_DATE}}] create | Knowledge Base Created

- Purpose: {{PURPOSE_STATEMENT}}
- Domain: {{DOMAIN_DESCRIPTION}}
- Entity types defined: {{ENTITY_TYPE_LIST}}
- Import procedures created: {{IMPORT_PROCEDURE_LIST}}
- qmd collection: `{{KB_NAME}}` configured and indexed
- Wiki structure scaffolded with directories: entities, concepts, synthesis{{ADDITIONAL_DIRS}}
